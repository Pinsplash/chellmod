...
public:
	...
	virtual float	NodeDistanceSqr( CAI_Node &node )
	{
		...
		{
			//Msg("NODE DISTANCE: %0.1f %0.1f %0.1f to %0.1f %0.1f %0.1f is %0.1f\n"
			//	, node.GetOrigin().x, node.GetOrigin().y, node.GetOrigin().z
			//	, m_pos.x, m_pos.y, m_pos.z, (node.GetOrigin() - m_pos).Length());
...
{
	//Msg("CAI_Network::ListNodesInBox\n");
	CNodeList result;
...
// UNDONE: Store the nodes in a tree and query the tree instead of the entire list!!!
	for ( int node = 0; node < m_iNumNodes; node++ )
	{
		//Msg("CAI_Network::ListNodesInBox loop node %i\n", node);
		CAI_Node *pNode = m_pAInode[node];
		...
		if ( flDist < flClosest )
		{
			//Msg("%i was closer (%0.1f) than previous closest (%0.1f)\n", node, flDist, flClosest);
			closest = node;
			flClosest = flDist;
		}
		...
		{
			//Msg("%i was added to node list\n", node);
			if (full)
			{
				//Msg("CAI_Network::ListNodesInBox list is full! %i removing old entry %i\n", node, result.ElementAtHead().nodeIndex);
				result.RemoveAtHead();
			}
	...
	list.RemoveAll();
	//Msg("List Nodes In Box: ");
	while ( result.Count() )
	{
		//Warning("%i ", result.ElementAtHead().nodeIndex);
...
int	CAI_Network::NearestNodeToPoint(CAI_BaseNPC *pNPC, const Vector &vecOrigin, bool bCheckVisibility, INearestNodeFilter *pFilter, bool bDebug)
{
	if (bDebug)
		Msg("CAI_Network::NearestNodeToPoint marker 1 origin %0.1f %0.1f %0.1f\n", vecOrigin.x, vecOrigin.y, vecOrigin.z);
	...
	{
		if (bDebug)
			Warning("CAI_Network::NearestNodeToPoint - no nodes found, wtf?\n");
		return NO_NODE;
	}
	...
	{
		if (bDebug)
			Msg("CAI_Network::NearestNodeToPoint marker 3\n");
		if ( bCheckVisibility )
		{
			if (bDebug)
				Msg("CAI_Network::NearestNodeToPoint marker 4\n");
			trace_t tr;
			...
			if (bDebug)
				Msg("AI TraceLine A to nearest node: FR: %f START: %0.1f %0.1f %0.1f END: %0.1f %0.1f %0.1f\n", tr.fraction, vecOrigin[0], vecOrigin[1], vecOrigin[2], vTestLoc[0], vTestLoc[1], vTestLoc[2]);

			if (tr.fraction != 1.0)
			{
				if (bDebug)
					Msg("AI TraceLine A to nearest node failed: FR: %f START: %0.1f %0.1f %0.1f END: %0.1f %0.1f %0.1f\n", tr.fraction, vecOrigin[0], vecOrigin[1], vecOrigin[2], vTestLoc[0], vTestLoc[1], vTestLoc[2]);
				cachedNode = NO_NODE;
			}
		}
		...
		{
			if (bDebug)
				Msg("CAI_Network::NearestNodeToPoint marker 6\n");
...
#endif
		//FIXFOR C6255
	AI_NearNode_t *pBuffer = (AI_NearNode_t *)stackmalloc( sizeof(AI_NearNode_t) * MAX_NEAR_NODES );
	...
	{
		if (bDebug)
			Msg("CAI_Network::NearestNodeToPoint marker 7\n");
	...
	{
		smallest = list.ElementAtHead().nodeIndex;
		if (bDebug)
			Msg("Got into main part of NearestNodeToPoint. (%i)\n", smallest);

		// Check not already rejected above
		if (smallest == cachedNode)
		{
			if (bDebug)
				Msg("CAI_Network::NearestNodeToPoint marker 8 (%i)\n", smallest);
			continue;
		}
		...
		{
			if (bDebug)
				Msg("CAI_Network::NearestNodeToPoint marker 9 (%i) - node can't be used by us!\n", smallest);
			continue;
		}
		...
		{
			if (bDebug)
				Msg("CAI_Network::NearestNodeToPoint marker 10 (%i) checking LOS\n", smallest);
			trace_t tr;

			Vector vTestLoc = ( pNPC ) ? 
								m_pAInode[smallest]->GetPosition(pNPC->GetHullType()) + pNPC->GetNodeViewOffset() : 
								m_pAInode[smallest]->GetOrigin() + Vector(0,0,1);//PIN: raise this position up once to prevent problems when looking for nearest node to portals. should i be worried this has to be done?
			...
			if (bDebug)
				Msg("AI TraceLine B to nearest node: FR: %f START: %0.1f %0.1f %0.1f END: %0.1f %0.1f %0.1f\n", tr.fraction, vecVisOrigin[0], vecVisOrigin[1], vecVisOrigin[2], vTestLoc[0], vTestLoc[1], vTestLoc[2]);

			if (tr.fraction != 1.0)
			{
				if (bDebug)
					Msg("AI TraceLine B to nearest node failed: FR: %f START: %0.1f %0.1f %0.1f END: %0.1f %0.1f %0.1f\n", tr.fraction, vecVisOrigin[0], vecVisOrigin[1], vecVisOrigin[2], vTestLoc[0], vTestLoc[1], vTestLoc[2]);
				continue;
			}
			if (bDebug)
				Msg("Has LOS\n");
		...
		{
			if (bDebug)
				Msg("CAI_Network::NearestNodeToPoint marker 11 valid check (%i)\n", smallest);
			if ( !pFilter->IsValid( m_pAInode[smallest] ) )//check CPathfindNearestNodeFilter's IsValid()
			{
				if (bDebug)
					Msg("CAI_Network::NearestNodeToPoint marker 12 (node does not pass filter) (%i)\n", smallest);
				if (!pFilter->ShouldContinue())
				{
					if (bDebug)
						Msg("CAI_Network::NearestNodeToPoint marker 13 (%i)\n", smallest);
					break;
				}
				continue;
			}
		...
		if (bDebug)
			Warning("NearestNodeToPoint success with (%i)\n", smallest);
		return smallest;
	}
	...
	if (bDebug)
		Warning("NearestNodeToPoint failed with (%i)\n", smallest);
	return NO_NODE;
}
...
int	CAI_Network::NearestNodeToPoint(const Vector &vPosition, bool bCheckVisibility, bool bDebug)
{
	return NearestNodeToPoint(NULL, vPosition, bCheckVisibility, bDebug);
}
...
{
	//Msg("CAI_Network::SetCachedNearestNode marker 1\n");
	if (ai_no_node_cache.GetBool())
	{
		//Msg("CAI_Network::SetCachedNearestNode marker 2\n");
		return;
	}
	...
	{
		//Msg("CAI_Network::SetCachedNearestNode marker 3\n");
		m_iNearestCacheNext = NEARNODE_CACHE_SIZE - 1;
	}
}
...
CAI_Link *CAI_Network::CreateLink( int srcID, int destID, CAI_DynamicLink *pDynamicLink )
{
	...
	{
		Warning( "Attempted to create link to node that doesn't exist\n" );
		return NULL;
	}

	if ( pSrcNode == pDestNode )
	{
		Warning("Attempted to link a node to itself\n");
		return NULL;
	}

	if ( pSrcNode->NumLinks() == AI_MAX_NODE_LINKS )
	{
		Warning("Node %d has too many links\n", srcID);
		return NULL;
	}
...
bool CAI_Network::IsConnected(int srcID, int destID, CAI_BaseNPC *pNPC, bool bTestOnly)
{
	...
	int srcSubZone = m_pAInode[srcID]->GetSubZone();
	int destSubZone = m_pAInode[destID]->GetSubZone();

	//Msg("CAI_Network::IsConnected &pNPC %s\n", &pNPC ? "TRUE" : "FALSE");
	//Msg("CAI_Network::IsConnected !(srcZone == destZone) %s (%i, %i)\n", !(srcZone == destZone) ? "TRUE" : "FALSE", srcZone, destZone);
	//Msg("CAI_Network::IsConnected &pNPC->m_bCanCrossZones %s\n", &pNPC->m_bCanCrossZones ? "TRUE" : "FALSE");

	//PIN: if we know how to handle crossing zones, then we don't need connections
	if (&pNPC && (!(srcZone == destZone) || !(srcSubZone == destSubZone)) && &pNPC->m_bCanCrossZones)
	{
		pNPC->NotifyNavZoneCross(srcZone, destZone, srcSubZone, destSubZone, bTestOnly);
		return true;
	}
...
//=============================================================================
void CAI_Network::CountZones()
{
	if (m_bZonesCounted)
		return;
	int nNodes = NumNodes();
	if (!nNodes)
		Assert(0);//no nodes in the map wtf r u doin mate

	int i = AI_NODE_FIRST_ZONE;
	int curZone = AI_NODE_FIRST_ZONE;
	int curSubZone = AI_NODE_FIRST_ZONE;
	for (i = 0; i < nNodes; i++)
	{
		if (m_pAInode[i]->GetZone() > curZone)
		{
			curZone = m_pAInode[i]->GetZone();
		}
		if (m_pAInode[i]->GetSubZone() > curSubZone)
		{
			curSubZone = m_pAInode[i]->GetSubZone();
		}
	}
	m_iZones = curZone;
	m_iSubZones = curSubZone;
	m_bZonesCounted = true;
}