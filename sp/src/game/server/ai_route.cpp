	...
	if (GetCurWaypoint()) 
	{
		...
		while (waypoint)
		{
			if (!waypoint->GetNext())
			{
				...
				//PIN: Shift the goal point to the ground for ground nodes. This is to fix chell trying to move to air to reach cubes that were leaving a dropper as she started moving (resulting in a falsely midair goalpos)
				if (waypoint->NavType() == NAV_GROUND)
				{
					trace_t tr;
					UTIL_TraceLine(m_goalPos, m_goalPos - Vector(0, 0, 1024), MASK_SOLID, NULL, COLLISION_GROUP_DEBRIS, &tr);
					Warning("Actual goal pos: %0.1f %0.1f %0.1f\n", tr.endpos.x, tr.endpos.y, tr.endpos.z);
					m_goalPos.z = tr.endpos.z;
				}
