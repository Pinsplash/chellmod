...
//https://github.com/NicolasDe/AlienSwarm/blob/c5a2d3fa853c726d040032ff2c7b90c8ed8d5d84/src/game/server/entitylist.cpp#L856
CBaseEntity	*CGlobalEntityList::FindEntityByOutputTarget(CBaseEntity *pStartEntity, string_t iTarget)
{
	const CEntInfo *pInfo = NULL;
	pInfo = pStartEntity ? GetEntInfoPtr(pStartEntity->GetRefEHandle())->m_pNext : FirstEntInfo();
	...
	for (; pInfo; pInfo = pInfo->m_pNext)
	{
		...
		while (dmap)
		{
			...
			for (int i = 0; i < fields; i++)
			{
				...
				{
					CBaseEntityOutput *pOutput = ent->FindNamedOutput(dataDesc->externalName);
