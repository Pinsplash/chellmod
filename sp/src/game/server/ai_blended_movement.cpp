		...
		else
		{
			...
			if (flNeededAccel < idealAccel)
			{
				...
				if (t1 > 0.0 && t2 > 0.0)
				{
					...
					*/
					//FIXFOR C6246
					float aNew = d1 / m_scriptMove[i].flDist;
					script.Init();
					script.vecLocation = m_scriptMove[i + 1].vecLocation * aNew + m_scriptMove[i].vecLocation * (1 - aNew);
