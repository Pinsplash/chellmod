	...
	case CChoreoEvent::GENERIC:
		{
			...
			{
				//FIXFOR C6246
				CBaseEntity *pTargetEnt = FindNamedEntity(event->GetParameters2());

				if (pTargetEnt)
				{
					info->m_nType = SCENE_AI_IGNORECOLLISION;
					info->m_hTarget = pTargetEnt;
					float remaining = event->GetEndTime() - scene->GetTime();
					NPCPhysics_CreateSolver(this, pTargetEnt, true, remaining);
	...
	case CChoreoEvent::FACE: 
		{
			...
			if (!info->m_bStarted)
			{
				info->m_flInitialYaw = GetAbsAngles().y;
			...
			diff = UTIL_AngleDiff(info->m_flTargetYaw, GetAbsAngles().y);
			if (diff < 0)
			{
				diff = -diff;
				dir = -1;
...
{ 
	if (!(m_fLatchedPositions & HUMANOID_LATCHED_HEAD))
	{
		...
		if (!IsPlayerPilot() && (!HasCondition( COND_IN_PVS ) || !GetAttachment( "eyes", m_latchedEyeOrigin, &m_latchedHeadDirection )))
		{
			m_latchedEyeOrigin = BaseClass::EyePosition( );
			AngleVectors(GetAbsAngles(), &m_latchedHeadDirection);
		}
		// clear out eye latch
		m_fLatchedPositions &= ~(HUMANOID_LATCHED_EYE);
...
{
	float diff = AngleDiff(flYaw, GetAbsAngles().y);
...
bool CAI_BaseActor::SetAccumulatedYawAndUpdate( void )
{
	if (m_flAccumYawScale > 0.0)
	{
		...
		float facing = GetAbsAngles().y + diff;
