...
{
	Msg("Call to Util_SetOrigin made from CAI_BehaviorHost<BASE_NPC>::BackBridge_Teleport! If this message was caused by chell, we have to reimplement\n");
...
{
	//PIN: This eventually leads down to a call that sets chell's local angles, so it could potentially, just maybe, have something to do with her teleporting
	//when trying to GoToEntity. I feel like this isn't the cause though because alyx works fine and gave similar results. Also, it only runs when initializing?
	if ( m_pCurBehavior )
	{
		//Msg("Call to Util_SetOrigin made from CAI_BehaviorHost<BASE_NPC>::Teleport (1)! If this message was caused by chell, we have to reimplement\n");
		m_pCurBehavior->BridgeTeleport( newPosition, newAngles, newVelocity );
		return;
	}
	//Msg("Call to Util_SetOrigin made from CAI_BehaviorHost<BASE_NPC>::Teleport (2)! If this message was caused by chell, we have to reimplement\n");
	//Msg("CAI_BehaviorHost<BASE_NPC>::Teleport %0.1f %0.1f %0.1f\n", newPosition[0], newPosition[1], newPosition[2]);
	BaseClass::Teleport(newPosition, newAngles, newVelocity);
}
