	...
	case TASK_FIND_COVER_FROM_ORIGIN:
		{
			...
			if (GetTacticalServices()->FindCoverPos(GetAbsOrigin(), EyePosition(), 0, CoverRadius(), &coverPos))
			{
...
//==================================================
	...
	case TASK_GET_PATH_TO_TARGET_WEAPON:
		{
			...
			else 
			{
				...
				int node = GetNavigator()->GetNetwork()->NearestNodeToPoint( this, m_hTargetEnt->GetAbsOrigin(), false, false);
	...
	case TASK_RUN_PATH_FLEE:
		{
			Vector vecDiff;
			vecDiff = GetAbsOrigin() - GetNavigator()->GetGoalPos();
	...
	case TASK_STRAFE_PATH:
		{
			...
			vec2DirToPoint = (GetNavigator()->GetCurWaypointPos() - GetAbsOrigin()).AsVector2D();
...
void CAI_BaseNPC::RunTask( const Task_t *pTask )
{
	...
	case TASK_FACE_PATH:
		{
			// If the yaw is locked, this function will not act correctly
			//Assert( GetMotor()->IsYawLocked() == false );//well fuck you
	...
	case TASK_MOVE_TO_GOAL_RANGE:
		{
			...
			else
			{
				...
				distance = fabs(vecGoalPos.z - GetAbsOrigin().z);
				if ( distance < pTask->flTaskData )
				{
					distance = (vecGoalPos - GetAbsOrigin()).Length2D();
				}
				else
				{
				...
				if ( GetNavigator()->GetNavType() != NAV_JUMP )
				{
					...
					{
						distance = (pTarget->GetAbsOrigin() - GetAbsOrigin()).Length2D();
			...
			case 2:
				{
					...
					if (GetTacticalServices()->FindCoverPos(GetAbsOrigin(), EyePosition(), 0, CoverRadius(), &coverPos) && IsValidMoveAwayDest(GetNavigator()->GetGoalPos()))
					{
