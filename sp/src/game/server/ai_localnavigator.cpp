...
bool CAI_LocalNavigator::MoveCalcDirect( AILocalMoveGoal_t *pMoveGoal, bool bOnlyCurThink, float *pDistClear, AIMoveResult_t *pResult )
{
	...
	if ( pMoveGoal->speed )
	{
		...
		{
			testPos = GetAbsOrigin() + pMoveGoal->dir * moveThisInterval;
			bTraceClear = GetMoveProbe()->MoveLimit(pMoveGoal->navType, GetAbsOrigin(), testPos,
		...
		if ( bTraceClear )
		{
			if ( m_FullDirectTimer.Expired() )
			{
				testPos = GetAbsOrigin() + pMoveGoal->dir * checkDist;//More instances of changing GetLocalOrigin to Abs to account for chell being parented! oh boy!
	...
	else
	{
		...
		Msg("MoveCalcDirect gave bad result because pMoveGoal->speed was false");
...
{
	//Msg("CAI_LocalNavigator::MoveCalcRaw marker 1\n");
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 2\n");
		SetSolveCookie();
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 3\n");
		//Msg("bDirectClear: %s bShouldSteer: %s\n", bDirectClear ? "TRUE" : "FALSE", bShouldSteer ? "TRUE" : "FALSE");
		//Msg("!(pMoveGoal->flags & AILMG_NO_STEER) is %s HaveObstacles() is %s\n", 
		//	!(pMoveGoal->flags & AILMG_NO_STEER) ? "TRUE" : "FALSE", HaveObstacles() ? "TRUE" : "FALSE");
		SetSolveCookie();
		return DbgResult( result );
	}
	...
	if ( bShouldSteer )
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 4 result: %i\n", result);
		if ( !bDirectClear )
		{
			//Msg("CAI_LocalNavigator::MoveCalcRaw marker 5 result: %i\n", result);
			if ( OnObstructionPreSteer( pMoveGoal, distClear, &result ) )
			{
				//Msg("CAI_LocalNavigator::MoveCalcRaw marker 6\n");
				SetSolveCookie();
				return DbgResult( result );
			}
		}
		...
		{
			//Msg("CAI_LocalNavigator::MoveCalcRaw marker 7\n");
			SetSolveCookie();
			return DbgResult( result );			
		}
	}
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 8\n");
		SetSolveCookie();
		return DbgResult( result );
	}
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 9\n");
		SetSolveCookie();
		return DbgResult( result );
	}
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 10 result: %i\n", result);
		if ( OnInsufficientStopDist( pMoveGoal, distClear, &result ) )
		{
			//Msg("CAI_LocalNavigator::MoveCalcRaw marker 11 result: %i\n", result);
			SetSolveCookie();
			return DbgResult( result );
		}
		...
		{
			//Msg("CAI_LocalNavigator::MoveCalcRaw marker 12\n");
			SetSolveCookie();
			return DbgResult( result );
		}
	}
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalcRaw marker 13\n");
		SetSolveCookie();
	...
	//Msg("CAI_LocalNavigator::MoveCalcRaw marker 14\n");
	DebugNoteMovementFailure();
	SetSolveCookie();
...
{
	//Msg("CAI_LocalNavigator::MoveCalc marker 1\n");
	...
	{
		//Msg("CAI_LocalNavigator::MoveCalc marker 2\n");
	...
	if ( result == AIMR_OK)
	{
		//Msg("CAI_LocalNavigator::MoveCalc marker 3\n");
		float interval = GetOuter()->GetMotor()->GetMoveInterval();
		float currentYaw = UTIL_AngleMod( GetAbsAngles().y );
		float goalYaw;
		float deltaYaw;
		float speed;
		float clampedYaw;
		...
		if ( deltaYaw > 15 )
		{
			//Msg("CAI_LocalNavigator::MoveCalc marker 4\n");
			...
			{
				//Msg("CAI_LocalNavigator::MoveCalc marker 5\n");
				pMoveGoal->facing = UTIL_YawToVector( clampedYaw );
			}
		}
	}

	//Msg("CAI_LocalNavigator::MoveCalc marker 6\n");
	return result;
}
//-----------------------------------------------------------------------------
