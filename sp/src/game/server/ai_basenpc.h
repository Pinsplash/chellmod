...
struct portalShot_t//Stuff we need to know about executing a portal shot.
{
	CAI_Node *pSrcNode = NULL;//Node where the shot should be fired from
	CAI_Node *pDestNode = NULL;//Node closest to the newly-made portal
	Vector vecPos = Vector(0, 0, 0);//put the portal here
	bool bLOS = false;//if false the shot is impossible, quick way to tell if a shot is not possible
	float flPlacementSuccess = PORTAL_FIZZLE_UNKNOWN;
	float flYaw = 0;
	bool bUseYaw = false;
};
...
public:
	...
	//
	CBaseEntity *m_hPortal1;
	CBaseEntity *m_hPortal2;
	CAI_Node *m_pPortal1Node;//Node closest to blue portal
	CAI_Node *m_pPortal2Node;//Node closest to orange portal
	...
	bool				m_bCanCrossZones;//PIN: if we know how to handle crossing zones, then we don't need connections
	virtual void		NotifyNavZoneCross(int srcZone, int destZone, int srcSubZone, int destSubZone, bool bTestOnly);//PIN: if we know how to handle crossing zones, then we don't need connections
	bool				m_bSolveBlockers;//PIN: let the NPC intelligently remove the blocker based on AI
	virtual int			NotifyNavBlocker(CBaseEntity *pEnt);//PIN: let the NPC intelligently remove the blocker based on AI
	virtual void		UpdateGunKnowledge(CBaseCombatWeapon *pWeapon);//PIN: Here as an attempt to fix a stupid error
	virtual void		UpdatePortalCross();//PIN: Here as an attempt to fix a stupid error
	virtual void		ChellCollide(int index, gamevcollisionevent_t *pEvent);//PIN: Here as an attempt to fix a stupid error
	virtual bool		IsPlayerPilot();//PIN: Basically explicitly tell engine we're chell to make exceptions
	virtual bool		IsPlayerPilot() const;//PIN: Basically explicitly tell engine we're chell to make exceptions
...
public:
	...
	//-----------------------------------------------------
	static CAI_GlobalNamespace gm_SquadSlotNamespace;//move protected line back up if issues happen
protected:
...
{
	Msg("&CAI_Component::GetLocalOrigin %0.1f %0.1f %0.1f\n", GetOuter()->GetLocalOrigin()[0], GetOuter()->GetLocalOrigin()[1], GetOuter()->GetLocalOrigin()[2]);
...
{
	Msg("CAI_Component::SetLocalOrigin %0.1f %0.1f %0.1f\n", origin[0], origin[1], origin[2]);
	GetOuter()->SetLocalOrigin(origin);
}

//-----------------------------------------------------------------------------

inline void CAI_Component::SetAbsOrigin(const Vector &origin)
{
	GetOuter()->SetAbsOrigin(origin);
}
...
{
	Msg("CAI_Component::SetLocalAngles (%s) %0.1f %0.1f %0.1f\n", GetOuter()->GetClassname(), angles[0], angles[1], angles[2]);
...
{
	Msg("&CAI_Component::GetLocalAngles (%s) L %0.1f %0.1f %0.1f A %0.1f %0.1f %0.1f\n", STRING(GetOuter()->m_iClassname), GetOuter()->GetLocalAngles()[0], GetOuter()->GetLocalAngles()[1], GetOuter()->GetLocalAngles()[2]
		, GetOuter()->GetAbsAngles()[0], GetOuter()->GetAbsAngles()[1], GetOuter()->GetAbsAngles()[2]);
