...
public:
	...
	AI_Waypoint_t*	FindBestPath		(int startID, int endID, bool bNoPortals);
	...
	AI_Waypoint_t *BuildRoute(const Vector &vStart, const Vector &vEnd, CBaseEntity *pTarget, float goalTolerance, bool bLocalSucceedOnWithinTolerance, Navigation_t curNavType = NAV_NONE, bool bNoPortals = false);
	...
	virtual AI_Waypoint_t *BuildNodeRoute( const Vector &vStart, const Vector &vEnd, int buildFlags, float goalTolerance, bool bNoPortals);
