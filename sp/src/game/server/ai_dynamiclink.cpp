				...
				if ( iLinkDest > i )
				{
					...
					if ( origin.DistToSqr(originOther) < MinDistCareSq )
					{
						if ( IsBoxIntersectingRay( vAbsMins, vAbsMaxs, nodeOrigin, originOther - nodeOrigin ) )
						{
							...
							//FIXFOR C6246
							CAI_DynamicLink *pDyLink = (CAI_DynamicLink *)CreateEntityByName( "info_node_link" );
							pDyLink->m_nSrcID = i;
							pDyLink->m_nDestID = iLinkDest;
							pDyLink->m_nSrcEditID = g_pAINetworkManager->GetEditOps()->GetWCIdFromNodeId(pDyLink->m_nSrcID);
							pDyLink->m_nDestEditID = g_pAINetworkManager->GetEditOps()->GetWCIdFromNodeId(pDyLink->m_nDestID);
							pDyLink->m_nLinkState = m_nLinkState;
							pDyLink->m_strAllowUse = m_strAllowUse;
							pDyLink->m_bInvertAllow = m_bInvertAllow;
							pDyLink->m_bFixedUpIds = true;
							pDyLink->m_bNotSaved = true;

							pDyLink->Spawn();
							m_ControlledLinks.AddToTail(pDyLink);
						}
					}
				}
			}
