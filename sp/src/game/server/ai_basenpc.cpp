...
void CAI_BaseNPC::UpdateEfficiency( bool bInPVS )
{
	...
	if ( GetSoundInterests() & SOUND_DANGER )
	{
		...
		while (iSound != SOUNDLIST_EMPTY)
		{
			...
			//FIXFOR C6011
			if (pCurrentSound != NULL && pCurrentSound->NextSound() != NULL)
			{
				iSound = pCurrentSound->NextSound();
			}
			else
			{
				Warning("C6011 avoidance ai_basenpc.cpp line 3229\n");
...
void CAI_BaseNPC::CollectShotStats( const Vector &vecShootOrigin, const Vector &vecShootDir )
{
	...
	if( ai_shot_stats.GetBool() != 0 && GetEnemy()->IsPlayer() )
	{
		...
		for( int i = 0 ; i < iterations ; i++ )
		{
			...
			//FIXFOR C6246
			Vector shootDir = vecProjectedPosition - vecShootOrigin;
			VectorNormalize(shootDir);
			manipulator.SetShootDir(shootDir);
		}
...
void CAI_BaseNPC::OnScheduleChange ( void )
{
	...
	//PIN: lol! test 02 is having an issue where a path is constantly regenerated because of this line. Chell is the only walking npc in this mod, sooooo...
	GetNavigator()->ClearGoal();
	...
	if ( pOldCine && pOldCine->m_iszPlay != NULL_STRING && pOldCine->PlayedSequence() )
	{
		if ( !pOldCine->HasSpawnFlags(SF_SCRIPT_DONT_TELEPORT_AT_END) )
		{
			...
			if ((oldOrigin - origin).Length() > 8.0)
			{
				...
				Msg("Call to Util_SetOrigin made from CAI_BaseNPC::CineCleanup! If this message was caused by chell, we have to reimplement\n");
...
void CAI_BaseNPC::CascadePlayerPush( const Vector &push, const Vector &pushOrigin )
{
	...
	for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
	{
		...
		//FIXFOR C6011
		if (pOther != this && pOther != NULL && pOther->IRelationType(this) == D_LI && !pOther->HasCondition(COND_PLAYER_PUSHING))
		{
...
bool CAI_BaseNPC::FindNearestValidGoalPos( const Vector &vTestPoint, Vector *pResult )
{
	...
	if ( vCandidate != vec3_invalid )
	{
		AI_Waypoint_t *pPathToPoint = GetPathfinder()->BuildRoute(GetAbsOrigin(), vCandidate, AI_GetSinglePlayer(), 5 * 12, true, NAV_NONE);
		if ( pPathToPoint )
		{
...
bool CAI_BaseNPC::CouldShootIfCrouching( CBaseEntity *pTarget )
{
	...
	{
		vecTarget = pTarget->BodyTarget(GetAbsOrigin());
	}

	bool bResult = WeaponLOSCondition(GetAbsOrigin(), vecTarget, false);
...
void CAI_BaseNPC::NotifyNavZoneCross(int srcZone, int destZone, int srcSubZone, int destSubZone, bool bTestOnly)
{
	Msg("CAI_BaseNPC::NotifyNavZoneCross\n");
}

int CAI_BaseNPC::NotifyNavBlocker(CBaseEntity *pEnt)
{
	Msg("CAI_BaseNPC::NotifyNavBlocker\n");
	return SHOULDHIT_DEFAULT;
}

bool CAI_BaseNPC::IsPlayerPilot()
{
	return false;
}

bool CAI_BaseNPC::IsPlayerPilot() const
{
	return false;
}

void CAI_BaseNPC::PilotJump()
{
	return;
}

void CAI_BaseNPC::UpdateGunKnowledge(CBaseCombatWeapon *pWeapon)
{
	return;
}

void CAI_BaseNPC::UpdatePortalCross()
{
	return;
}

void CAI_BaseNPC::ChellCollide(int index, gamevcollisionevent_t *pEvent)
{
	return;
}