...
void CAI_NetworkManager::SaveNetworkGraph( void )
{
	...
	for ( node = 0; node < m_pNetwork->m_iNumNodes; node++)
	{
		...
		Assert(pNode->GetSubZone() != AI_NODE_ZONE_UNKNOWN);
...
void CAI_NetworkManager::LoadNetworkGraph( void )
{
	...
	for ( node = 0; node < numNodes; node++)
	{
		...
		new_node->m_subzone = buf.GetShort();
	}
...
void CAI_NetworkEditTools::DrawAINetworkOverlay(void)
{
	...
	if (m_debugNetOverlays & bits_debugOverlayGrid)
	{
		...
		if (pPlayer)
		{
			...
			{
				NDebugOverlay::Grid( tr.endpos + Vector(0,0,1) );//PIN: Fix?
			}
		}
	}
		...
		else if (m_iGConnectivityNode != NO_NODE)
		{
			for (int node=0;node<m_pNetwork->NumNodes();node++) 
			{
				if ( m_pNetwork->IsConnected( m_iGConnectivityNode, node) )
				{
					...
					NDebugOverlay::Line(srcPos, desPos, 255,0,255, false,0);//PIN: Fix?
				}
			}
		}
	}
			...
			for (int node=startDrawNode;node<endDrawNode;node++) 
			{
				for (int link=0;link<pAINode[node]->NumLinks();link++) 
				{
					...
					if (pAINode[node]->GetLinkByIndex(link)->DestNodeID(node) < node) 
					{
						...
						{	
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 100,255,255,20,flDrawDuration);//PIN: Fix?
						}
						...
						{	
							...
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 255,0,255,20,flDrawDuration);//PIN: Fix?
						}

						if (pAINode[node]->GetLinkByIndex(link)->m_iAcceptedMoveTypes[m_iHullDrawNum] & bits_CAP_MOVE_GROUND) 
						{	
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 0,255,50,20,flDrawDuration);//PIN: Fix?
						}

						else if (pAINode[node]->GetLinkByIndex(link)->m_iAcceptedMoveTypes[m_iHullDrawNum] & bits_CAP_MOVE_JUMP) 
						{	
							NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction, 0,0,255,20,flDrawDuration);//PIN: Fix?
						}
					}
				}
			}
	...
	{
		CAI_HintManager::DrawHintOverlays(flDrawDuration);//PIN: Fix?
	}
		...
		for (int node=startDrawNode;node<endDrawNode;node++) {
			...
			if (pAINode[node]->GetType() != NODE_DELETED)
			{
					...
					else
					{
						for (int link=0;link<pAINode[node]->NumLinks();link++) {
							...
							if (pAINode[node]->GetLinkByIndex(link)->DestNodeID(node) < node)
							{
								...
								{
									NDebugOverlay::Line(srcPos, desPos, 255,0,0, false, flDrawDuration);//PIN: Fix?
								}
								// Draw in grey if link turned off
								else if (linkInfo & bits_LINK_OFF)						
								{
									NDebugOverlay::Line(srcPos, desPos, 100,100,100, false, flDrawDuration);//PIN: Fix?
								}
								else if ((m_debugNetOverlays & bits_debugOverlayFlyConnections) && (moveTypes & bits_CAP_MOVE_FLY))
								{	
									NDebugOverlay::Line(srcPos, desPos, 100,255,255, false, flDrawDuration);//PIN: Fix?
								}
								else if (moveTypes & bits_CAP_MOVE_CLIMB) 
								{	
									NDebugOverlay::Line(srcPos, desPos, 255,0,255, false, flDrawDuration);//PIN: Fix?
								}
								else if (moveTypes & bits_CAP_MOVE_GROUND) 
								{	
									NDebugOverlay::Line(srcPos, desPos, 0,255,50, false, flDrawDuration);//PIN: Fix?
								}
								else if ((m_debugNetOverlays & bits_debugOverlayJumpConnections) && (moveTypes & bits_CAP_MOVE_JUMP) )
								{	
									NDebugOverlay::Line(srcPos, desPos, 0,0,255, false, flDrawDuration);//PIN: Fix?
								}
								else  
								{	// Dark red if this hull can't use
									...
									if ( ( isFly && (m_debugNetOverlays & bits_debugOverlayFlyConnections) ) ||
										 ...
										 ( !isFly && !isJump ) )
									{
										NDebugOverlay::Line(srcPos, desPos, 100,25,25, false, flDrawDuration);//PIN: Fix?
									}
								}
							}
						}
				...
				if (m_debugNetOverlays & bits_debugOverlayNodes) 
				{
					...
					NDebugOverlay::Box(nodePos, Vector(-5,-5,-5), Vector(5,5,5), r,g,b,0,flDrawDuration);//PIN: Fix?
					...
					if (pAINode[node]->GetType() == NODE_CLIMB)
					{
						...
						NDebugOverlay::Line(nodePos, nodePos+offsetDir, r,g,b,false,flDrawDuration);//PIN: Fix?
					}

					if ( pAINode[node]->GetHint() )
					{
						NDebugOverlay::Box( nodePos, Vector(-7,-7,-7), Vector(7,7,7), 255,255,0,0,flDrawDuration);//PIN: Fix?
					}
					...
					if (m_debugNetOverlays & bits_debugOverlayNodesLev2)
					{
						...
						if ( m_pNodeIndexTable )
							msg.sprintf("%i (wc:%i; z:%i, sz:%i)", node, m_pNodeIndexTable[pAINode[node]->GetId()], pAINode[node]->GetZone(), pAINode[node]->GetSubZone());
						else
							msg.sprintf("%i (z:%i, sz:%i)", node, pAINode[node]->GetZone(), pAINode[node]->GetSubZone());

						Vector loc = nodePos;
						loc.x+=6;
						loc.y+=6;
						loc.z+=6;
						NDebugOverlay::Text( loc, msg, true, flDrawDuration);//PIN: Fix?
...
void CAI_NetworkBuilder::FloodFillSubZone(CAI_Node **ppNodes, CAI_Node *pNode, int subzone)
{
	Msg("SUB ZONE FILL: Node %i assigned to subzone %i\n", pNode->GetId(), subzone);
	pNode->SetSubZone(subzone);
	...
	for (int link = 0; link < pNode->NumLinks(); link++)
	{
		...
		//for (int i = HULL_HUMAN; i < NUM_HULLS; i++)
		//{
			CAI_Node *pTempLinkedNode = (pLink->m_iDestID == pNode->GetId()) ? ppNodes[pLink->m_iSrcID] : ppNodes[pLink->m_iDestID];
			if (!(pLink->m_iAcceptedMoveTypes[HULL_HUMAN] & ~bits_CAP_MOVE_JUMP))//Apparently we have to do that double negation.
			{
				Warning("SUB ZONE FILL: Is link from %i to %i jumping with hull %i? Yes, do not give node %i the subzone %i\n", pNode->GetId(), pTempLinkedNode->GetId(), HULL_HUMAN, pTempLinkedNode->GetId(), subzone);
				continue;//Subzones don't go between jump connections.
			}
			Msg("SUB ZONE FILL: Is link from %i to %i jumping with hull %i? No, give node %i the subzone %i\n", pNode->GetId(), pTempLinkedNode->GetId(), HULL_HUMAN, pTempLinkedNode->GetId(), subzone);
		//}
		CAI_Node *pLinkedNode = (pLink->m_iDestID == pNode->GetId()) ? ppNodes[pLink->m_iSrcID] : ppNodes[pLink->m_iDestID];
		if (pLinkedNode->GetSubZone() == AI_NODE_ZONE_UNKNOWN)
		{
			Msg("SUB ZONE FILL: Node %i does not have a subzone yet, adding it to %i\n", pLinkedNode->GetId(), subzone);
			FloodFillSubZone(ppNodes, pLinkedNode, subzone);
		}
		Warning("SUB ZONE FILL: Node %i already has a subzone: %i\n", pLinkedNode->GetId(), pLinkedNode->GetSubZone());

		Assert(pLinkedNode->GetSubZone() == pNode->GetSubZone() && pNode->GetSubZone() == subzone);
	}
}
...
void CAI_NetworkBuilder::InitZones( CAI_Network *pNetwork )
{
	...
	for (i = 0; i < nNodes; i++)
	{
		...
		ppNodes[i]->SetSubZone(AI_NODE_ZONE_UNKNOWN);
	}
	...
	{	
		if (ppNodes[i]->NumLinks() == 0)
		{
			...
			ppNodes[i]->SetSubZone(AI_NODE_ZONE_SOLO);
		}
	}
	...
	for (i = 0; i < nNodes; i++)
	{
		...
		if ( ppNodes[i]->GetSubZone() == AI_NODE_ZONE_UNKNOWN )
		{
			FloodFillSubZone( (CAI_Node **)ppNodes, ppNodes[i], curSubZone );
			curSubZone++;
		}
	}
	pNetwork->m_iZones = curZone;
	pNetwork->m_iSubZones = curSubZone;

#ifdef DEBUG
	for (i = 0; i < nNodes; i++)
	{	
		Assert(ppNodes[i]->GetZone() != AI_NODE_ZONE_UNKNOWN);
		Assert(ppNodes[i]->GetSubZone() != AI_NODE_ZONE_UNKNOWN);
	}
#endif
}
...
void CAI_NetworkBuilder::Rebuild( CAI_Network *pNetwork )
{
	...
	for (i = 0; i < nNodes; i++)
	{
		...
		if (ppNodes[i]->m_eNodeInfo & bits_NODE_WC_CHANGED)
		{
			...
			ppNodes[i]->SetSubZone(AI_NODE_ZONE_UNIVERSAL);
			for (int node = 0; node < nNodes; node++)
			{
				if ( ppNodes[node]->GetType() == NODE_AIR )
				{
					if ((ppNodes[node]->GetOrigin() - vRebuildPos).LengthSqr() < MAX_AIR_NODE_LINK_DIST_SQ)
					{
						...
						ppNodes[node]->SetSubZone(AI_NODE_ZONE_UNIVERSAL);
					}
				}
				else
				{
