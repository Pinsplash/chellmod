...
void CBasePlayer::NotifySystemEvent(CBaseEntity *pNotify, notify_system_event_t eventType, const notify_system_event_params_t &params)
{
	//We just crossed through a portal
	if (eventType == NOTIFY_EVENT_TELEPORT)
	{
		Warning("Player found portal crossed... telling chell\n");
		CBaseEntity *pEnt = gEntList.FindEntityByClassname(NULL, "npc_chell");
		if (pEnt)
		{
			CAI_BaseNPC *pChell = static_cast<CAI_BaseNPC*>(pEnt);
			if (pChell)
			{
				Warning("Player found chell\n");
				pChell->UpdatePortalCross();
			}
		}
	}
...
void CBasePlayer::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if (pEnt)
	{
		...
		if (pChell)
		{
			pChell->ChellCollide(index, pEvent);
		}
	}
}
