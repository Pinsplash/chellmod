...
void CBaseAnimating::SetupBones( matrix3x4_t *pBoneToWorld, int boneMask )
{
	...
	else 
	{
		if ( m_pIk )
		{
			...
			//Msg("pos 3: %0.1f %0.1f %0.1f\n", pos->x, pos->y, pos->z);
			GetSkeleton( pStudioHdr, pos, q, boneMask );

			//Msg("pos 4: %0.1f %0.1f %0.1f\n", pos->x, pos->y, pos->z);
			m_pIk->UpdateTargets(pos, q, pBoneToWorld, boneComputed);
			CalculateIKLocks(gpGlobals->curtime);
			//Msg("pos 5: %0.1f %0.1f %0.1f\n", pos->x, pos->y, pos->z);
			m_pIk->SolveDependencies(pos, q, pBoneToWorld, boneComputed);
		}
		else
		{
			// Msg( "%.03f : %s:%s\n", gpGlobals->curtime, GetClassname(), GetEntityName().ToCStr() );
			//Msg("pos 6: %0.1f %0.1f %0.1f\n", pos->x, pos->y, pos->z);
			GetSkeleton(pStudioHdr, pos, q, boneMask);
		}
	}
