...
AIMotorMoveResult_t CAI_Motor::MoveGroundStep( const Vector &newPos, CBaseEntity *pMoveTarget, float yaw, bool bAsFarAsCan, bool bTestZ, AIMoveTrace_t *pTraceResult )
{
	...
	GetMoveProbe()->TestGroundMove(GetAbsOrigin(), newPos, MASK_NPCSOLID, testFlags, &moveTrace);
	if ( pTraceResult )
	{
	...
	{
#ifdef DEBUG
		//PIN: Changed to Abs on April 9 2022
		if ( GetMoveProbe()->CheckStandPosition( GetAbsOrigin(), MASK_NPCSOLID ) && !GetMoveProbe()->CheckStandPosition( moveTrace.vEndPosition, MASK_NPCSOLID ) )
		{
		...
		//Msg("UTIL_SetOrigin in CAI_Motor::MoveGroundStep USING NEW CODE %0.1f %0.1f %0.1f\n", moveTrace.vEndPosition[0], moveTrace.vEndPosition[1], moveTrace.vEndPosition[2]);
		if (!GetOuter()->GetParent())
		{
			DevMsg("NPC has no parent, running usual (new) step code\n");
			UTIL_SetAbsOrigin(GetOuter(), moveTrace.vEndPosition, true);
			//Apparently this line is the literal one that makes hl2 npcs move! dont break it
		}
		else
		{
			//DevMsg("NPC has a parent, will NOT run usual step code\n");
		}
		...
		if ( yaw != -1 )
		{
			...
			angles.y = yaw;
			//SetLocalAngles(angles);//PIN: unsure about changing this? Does GetOuter()->SetAbsAngles(angles) work here?
			Warning("CAI_Motor::MoveGroundStep did questionable setangles?\n");
			GetOuter()->SetAbsAngles(angles);
		}
		if ( bHitTarget )
...
AIMotorMoveResult_t CAI_Motor::MoveFlyExecute( const AILocalMoveGoal_t &move, AIMoveTrace_t *pTraceResult )
{
	...
	// The true argument here causes it to touch all triggers
	// in the volume swept from the previous position to the current position
	Msg("UTIL_SetOrigin in CAI_Motor::MoveFlyExecute %0.1f %0.1f %0.1f\n", moveTrace.vEndPosition[0], moveTrace.vEndPosition[1], moveTrace.vEndPosition[2]);
...
void CAI_Motor::MoveFacing( const AILocalMoveGoal_t &move )
{
	...
	else
	{
		...
		float flDiff = UTIL_AngleDiff(flMoveYaw, GetAbsAngles().y);
...
void CAI_Motor::UpdateYaw( int yawSpeed )
{
	...
	if (newYaw != current)
	{
		...
		angles.y = newYaw;
		//SetLocalAngles(angles);//PIN: unsure about changing this? Does GetOuter()->SetAbsAngles(angles) work here?
		Warning("CAI_Motor::UpdateYaw did questionable setangles? old yaw %0.1f new yaw %0.1f\n", GetAbsAngles()[1], angles[1]);
		GetOuter()->SetAbsAngles(angles);
	}
}
