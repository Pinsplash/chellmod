...
public:
	...
	bool			IsConnected(int srcID, int destID, CAI_BaseNPC *pNPC = NULL, bool bTestOnly = false);	// Use during run time
	...
	int				NearestNodeToPoint(CAI_BaseNPC* pNPC, const Vector &vecOrigin, bool bCheckVisiblity, INearestNodeFilter *pFilter, bool bDebug);
	int				NearestNodeToPoint(CAI_BaseNPC* pNPC, const Vector &vecOrigin, bool bCheckVisiblity, bool bDebug) { return NearestNodeToPoint(pNPC, vecOrigin, bCheckVisiblity, NULL, bDebug); }
	int				NearestNodeToPoint(const Vector &vPosition, bool bCheckVisiblity, bool bDebug);

	int				NumNodes() const 	{ Assert(m_iNumNodes > 0); return m_iNumNodes; }
	int				NumZones()  	{ CountZones(); return m_iZones; }
	int				NumSubZones()  	{ CountZones(); return m_iSubZones; }
	void			CountZones();
	int m_iZones;
	int m_iSubZones;
	bool m_bZonesCounted = false;
