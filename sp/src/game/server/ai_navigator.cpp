...
bool CAI_Navigator::SetGoal( const AI_NavGoal_t &goal, unsigned flags )
{
	...
	else
	{
		...
		SimplifyPath(!GetOuter()->IsPlayerPilot(), GetOuter()->IsPlayerPilot() ? -1 : goal.maxInitialSimplificationDist);
	}
...
bool CAI_Navigator::SetRandomGoal( const Vector &from, float minPathLength, const Vector &dir )
{
	...
	int fromNodeID = GetNetwork()->NearestNodeToPoint( GetOuter(), from, true, pFilter, false);
...
{
	//Msg("CAI_Navigator::StopMoving( %d )\n", bImmediate);
	if ( IsGoalSet() )
	{
...
{ 
	//Msg("(%s) Schedule change\n", GetOuter()->GetClassname());//spammy
}
...
{
	//Msg("(%s) New Goal\n", GetOuter()->GetClassname());//PIN: idk why this happens from ClearGoal? seems odd
	ResetCalculations();
	m_fNavComplete = true;
}
...
{
	//Msg("CAI_Navigator::TeleportAlongPath");
	while (GetPath()->GetCurWaypoint())
	{
		...
		if ( CanFitAtPosition( vTestPoint, MASK_NPCSOLID, false, false ) )
		{
			...
			{
				Msg("Call to Util_SetOrigin made from CAI_Navigator::TeleportAlongPath! If this message was caused by chell, we have to reimplement\n");
...
AIMoveResult_t CAI_Navigator::MoveJump()
{
	...
	{
		// DevMsg( "jump to %f %f %f landed %f %f %f", GetPath()->CurWaypointPos().x, GetPath()->CurWaypointPos().y, GetPath()->CurWaypointPos().z, GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z );
...
{
	//Msg("CAI_Navigator::OnInsufficientStopDist marker 1 result: %i\n", pResult);
	// Allow the NPC to override this behavior
	if ( GetOuter()->OnInsufficientStopDist( pMoveGoal, distClear, pResult ))
	{
		//Msg("CAI_Navigator::OnInsufficientStopDist marker 2 result: %i\n", pResult);
		DebugNoteMovementFailureIfBlocked( *pResult );
		return true;
	}
	...
	if ( !IsMovingOutOfWay( *pMoveGoal, distClear ) )
	{
		...
		{
			//Msg("CAI_Navigator::OnInsufficientStopDist marker 5 result: %i\n", pResult);
			// Try bumping to side
			...
			for ( int i = 1; i > -2; i -= 2 )
			{
				//Msg("CAI_Navigator::OnInsufficientStopDist marker 6 result: %i\n", pResult);
				Vector testLoc = GetAbsOrigin() + (vDeflection * GetHullWidth() * 2.0) * i;
				GetMoveProbe()->MoveLimit(GetNavType(), GetAbsOrigin(), testLoc, MASK_NPCSOLID, NULL, &moveTrace);
				if ( moveTrace.fStatus == AIMR_OK )
				{
					//Msg("CAI_Navigator::OnInsufficientStopDist marker 7 result: %i\n", pResult);
					...
					if ( moveTrace.fStatus == AIMR_OK )
					{
						//Msg("CAI_Navigator::OnInsufficientStopDist marker 8 result: %i\n", pResult);
		...
		if ( distClear < 1.0 )
		{
			//Msg("CAI_Navigator::OnInsufficientStopDist marker 9 result: %i\n", pResult);
		...
		//Msg("CAI_Navigator::OnInsufficientStopDist marker 10 result: %i\n", pResult);
		return false;
	}
	...
	*pResult = AIMR_OK;
	pMoveGoal->maxDist = distClear;
	pMoveGoal->flags |= AILMG_CONSUME_INTERVAL;
	//Msg("CAI_Navigator::OnInsufficientStopDist marker 11 result: %i\n", pResult);
	return true;
}
		...
		if ( pBlocker )
		{
			if ( m_hPeerWaitingOn != pBlocker || m_PeerWaitClearTimer.Expired() )
			{
				...
				if ( pBlocker->GetGroundEntity() == GetOuter() )
				{
					...
					{
						Msg("Chell calling UTIL_SetOrigin in CAI_Navigator::DelayNavigationFailure, probably causing issue\n");
...
{
	Warning("CAI_Navigator::OnMoveBlocked\n");
...
{
	//Msg("CAI_Navigator::MoveEnact marker 1\n");
	...
	if (result != AIMR_OK)
	{
		//Msg("CAI_Navigator::MoveEnact marker 2\n");
		m_hLastBlockingEnt = move.directTrace.pObstruction;
	}
	else
	{
		//Msg("CAI_Navigator::MoveEnact marker 3\n");
...
{
	//Msg("CAI_Navigator::MoveNormal marker 1\n");
	if (!PreMove())
	{
		//Msg("CAI_Navigator::MoveNormal marker 2\n");
		return AIMR_ILLEGAL;
	}
	if (ShouldTestFailMove())
	{
		//Msg("CAI_Navigator::MoveNormal marker 3\n");
		return AIMR_ILLEGAL;
	}
	...
	{
		//Msg("CAI_Navigator::MoveNormal marker 4\n");
		return result;
	}
	...
	{
		//Msg("CAI_Navigator::MoveNormal marker 5\n");
		if (GetActivity() == ACT_TRANSITION)
		{
			//Msg("CAI_Navigator::MoveNormal marker 6\n");
			return AIMR_OK;
		}
	...
	{
		//Msg("CAI_Navigator::MoveNormal marker 7\n");
		if ( ( GetAbsOrigin() - vStart ).Length() < 0.01 )
		{
			//Msg("CAI_Navigator::MoveNormal marker 8\n");
	...
	//Msg("CAI_Navigator::MoveNormal marker 9\n");
	return result;
}
...
{
	//Msg("CAI_Navigator::Move marker 1\n");
	if (flInterval > 1.0)
	{
		//Msg("CAI_Navigator::Move marker 2\n");
	...
	{
		//Msg("CAI_Navigator::Move marker 3\n");
		...
		if ( fShouldMove )
		{
			//Msg("CAI_Navigator::Move marker 8\n");
			...
			{
				Msg( "Move requested with no route!\n" );
				result = AINR_NO_GOAL;
			}
			else if (!GetPath()->GetCurWaypoint())
			{
				Msg( "Move goal with no route!\n" );
				GetPath()->Clear();
			...
			if ( result == AINR_OK )
			{
				//Msg("CAI_Navigator::Move marker 9\n");
				// ---------------------------------------------------------------------
				// If I've been asked to wait, let's wait
				// ---------------------------------------------------------------------
				if ( GetOuter()->ShouldMoveWait() )
				{
					//Msg("CAI_Navigator::Move marker 10\n");
					GetMotor()->MovePaused();
					return false;
				}
				...
				{
					//Msg("CAI_Navigator::Move loop running\n");
					if ( GetMotor()->GetMoveInterval() <= 0 )
					{
						//Msg("Got an ok moveresult\n");
						moveResult = AIMR_OK;
						break;
					}
					...
					{
						//Msg("CAI_Navigator::Move marker 11\n");
						GetMotor()->MoveStart();
					...
					{
						//Msg("CAI_Navigator::Move marker 12\n");
						m_hBigStepGroundEnt = NULL;
					}
					...
					{
					case NAV_CLIMB:
						//Msg("CAI_Navigator::Move marker 13\n");
						moveResult = MoveClimb();
						break;

					case NAV_JUMP:
						//Msg("CAI_Navigator::Move marker 14\n");
						moveResult = MoveJump();
						break;

					case NAV_GROUND:
					case NAV_FLY:
						//Msg("going to run movenormal\n");
					...
					{
						//Msg("CAI_Navigator::Move marker 15\n");
						bMoved = true;
					}
					...
					if ( nLoopCount > 16 )
					{
						//Msg( "ERROR: %s navigation not terminating. Possibly bad cyclical solving?\n", GetOuter()->GetDebugName() );
						moveResult = AIMR_ILLEGAL;
						...
						case NAV_GROUND:
						case NAV_FLY:
							//Msg("CAI_Navigator::Move marker 16\n");
					...
					if (moveResult != AIMR_BLOCKED_NPC || GetNavType() == NAV_CLIMB || GetNavType() == NAV_JUMP || GetPath()->CurWaypointNavType() == NAV_JUMP )
					{
						...
						if ( MarkCurWaypointFailedLink() )
						{
							...
							if ( pSavedWaypoints )
							{
								...
								{
									//Msg("----RECOVER STAGE 4A----\n");
									DeleteAll( pSavedWaypoints );
									bRecovered = true;
								}
								else
								{
									//Msg("----RECOVER STAGE 4B----\n");
		...
		else 
		{
			...
			//Msg("CAI_Navigator::Move marker 19\n");
			if ( GetNavType() == NAV_CLIMB )
			{
				//Msg("CAI_Navigator::Move marker 20\n");
				GetMotor()->MoveClimbStop();
				SetNavType( NAV_GROUND );
			}
			GetMotor()->MoveStop();
		...
		//Msg("CAI_Navigator::Move marker 21\n");
		return false;
	}

	//Msg("CAI_Navigator::Move marker 22\n");
...
bool CAI_Navigator::SimplifyPath( bool bFirstForPath, float scanDist )
{
	...
	//HACK: Chell thinks she can simplify through the chamber 01 glass. We should not have to do this.
	//This is not perfect.

	if (GetPath()->GetCurWaypoint() != NULL && GetPath()->GetCurWaypoint()->GetNext() != NULL)
	{
		trace_t tr;
		Vector pos1 = GetPath()->GetCurWaypoint()->GetPos() + Vector(0, 0, 18);
		Vector pos2 = GetPath()->GetCurWaypoint()->GetNext()->GetPos() + Vector(0, 0, 18);
		UTIL_TraceLine(pos1, pos2, MASK_NPCSOLID, UTIL_GetLocalPlayer(), COLLISION_GROUP_DEBRIS, &tr);
		if (tr.fraction != 1)
		{
			//Warning("Simplifying impossible - quick reject\n");
			return false;
		}
		else
		{
			//Warning("Simplifying possible - %0.1f %0.1f %0.1f to %0.1f %0.1f %0.1f\n", pos1.x, pos1.y, pos1.z, pos2.x, pos2.y, pos2.z);
		}
	}
	...
	if (navType == NAV_GROUND || navType == NAV_FLY)
	{
		...
		{
			//if (pCurWaypoint)
			//	Msg("Did not simplify on node %i\n", pCurWaypoint->iNodeID);
			return false;
		}
...
bool CAI_Navigator::CanFitAtNode(int nodeNum, unsigned int collisionMask ) 
{
	...
	if (pTestNode->GetType() == NODE_GROUND)
	{
		...
		{
			Warning("Can't fit at node %i because bad checkstandposition (ground)\n", nodeNum);
			return false;
		}
	}
		...
		(pTestNode->m_eNodeInfo & (bits_NODE_CLIMB_BOTTOM | bits_NODE_CLIMB_EXIT )))
	{
		if (!GetMoveProbe()->CheckStandPosition( startPos, collisionMask ))
		{
			Warning("Can't fit at node %i because bad checkstandposition (climb)\n", nodeNum);
			return false;
		}
	}
	...
	if (!CanFitAtPosition( startPos, collisionMask ))
	{
		...
		if (!CanFitAtPosition(startPos, collisionMask))
		{
			Warning("Can't fit at node %i because bad CanFitAtPosition\n", nodeNum);
			return false;
		}
	}
...
//			occured as stale so when then next node route is built that link
...
{
	//Warning("MarkCurWaypointFailedLink running - BAD\n");
	if (TestingSteering())
	{
		//Msg("MarkCurWaypointFailedLink marker 1\n");
		return false;
	}

	if (!m_fRememberStaleNodes)
	{
		//Msg("MarkCurWaypointFailedLink marker 2\n");
		return false;
	}
	...
	{
		//Msg("MarkCurWaypointFailedLink marker 3\n");
		return false;
	}
	bool didMark = false;
	...
	//Msg("MarkCurWaypointFailedLink startID: %i endID: %i\n", startID, endID);
	if ( endID != NO_NODE )
	{
		//Msg("MarkCurWaypointFailedLink marker 4\n");
		bool bBlockAll = false;

		if (m_hLastBlockingEnt)
			//Msg("Blocker is: %s\n", m_hLastBlockingEnt.Get()->GetClassname());
		...
		{
			//Msg("MarkCurWaypointFailedLink marker 5\n");
			...
			for ( int i = 0; i < 3; i++ )
			{
				//Msg("MarkCurWaypointFailedLink marker 6\n");
				if ( vecSize[i] > 40 )
				{
					//Msg("MarkCurWaypointFailedLink marker 7\n");
					bFoundLarge = true;
				}

				if ( vecSize[i] < 30 )
				{
					//Msg("MarkCurWaypointFailedLink marker 8\n");
					bFoundSmall = true;
					break;
				}
			}
			...
			{
				//Msg("MarkCurWaypointFailedLink marker 9\n");
				...
				if (tr.startsolid)
				{
					//Msg("MarkCurWaypointFailedLink marker 10\n");
					bBlockAll = true;
				}
			}
		}
		...
		if ( bBlockAll )
		{
			//Msg("MarkCurWaypointFailedLink marker 11\n");
			CAI_Node *pDestNode = GetNetwork()->GetNode( endID );
			for ( int i = 0; i < pDestNode->NumLinks(); i++ )
			{
				//Msg("MarkCurWaypointFailedLink marker 12\n");
	...
	//("MarkCurWaypointFailedLink marker 15\n");
	return didMark;
}
...
bool CAI_Navigator::DoFindPathToPos(void)
{
	...
	if (m_pClippedWaypoints->IsEmpty() || GetOuter()->IsPlayerPilot())
	{
	...
	AI_Waypoint_t *pFirstWaypoint = pPathfinder->BuildRoute(origin, actualGoalPos, pTarget, tolerance, m_bLocalSucceedOnWithinTolerance, GetNavType());
...
void CAI_Navigator::ClearPath( void )
{
	...
	if ( pWaypoint )
	{
		...
		{
			//Assert( m_PreviousMoveActivity > ACT_RESET );//PIN: ?
		}
