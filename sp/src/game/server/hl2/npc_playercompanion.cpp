...
int CNPC_PlayerCompanion::SelectSchedulePriorityAction()
{
	if ( GetGroundEntity() && !IsInAScript() )
	{
		...
		{
			//return SCHED_PC_GET_OFF_COMPANION;// PIN: How would this even happen in normal circumstances??
		}
...
void CNPC_PlayerCompanion::InputOutsideTransition( inputdata_t &inputdata )
{
	...
	{
		AI_Waypoint_t *pPathToPlayer = GetPathfinder()->BuildRoute( GetAbsOrigin(), playerPos, pPlayer, 0, false);
	...
	while ( pHint )
	{
		...
		if ( GetNavigator()->CanFitAtPosition( teleportLocation, MASK_NPCSOLID ) )
		{
			...
			{
				AI_Waypoint_t *pPathToTeleport = GetPathfinder()->BuildRoute( GetAbsOrigin(), teleportLocation, pPlayer, 0, false);
