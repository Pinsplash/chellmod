...
{
	return GetNetwork()->NearestNodeToPoint( GetOuter(), GetAbsOrigin(), true, false);
}
...
{
	return GetNetwork()->NearestNodeToPoint( GetOuter(), vecOrigin, true, false);
}
...
AI_Waypoint_t *CAI_Pathfinder::FindBestPath(int startID, int endID, bool bNoPortals) 
{
	//Msg("\nCAI_Pathfinder::FindBestPath trying %i to %i\n", startID, endID);
	...
	// ------------- INITIALIZE ------------------------
	//FIXFOR C6255
	float* nodeG = (float *)stackmalloc( nNodes * sizeof(float) );//PIN: Length of the path from the start to the given node
	float* nodeH = (float *)stackmalloc( nNodes * sizeof(float) );//PIN: How far the node is from the goal node. (Direct distance.)
	float* nodeF = (float *)stackmalloc( nNodes * sizeof(float) );//PIN: Sum of a node's G and H.
	int*   nodeP = (int *)stackmalloc( nNodes * sizeof(int) );//PIN: The node that a given node would be approched from in the path.
	//FIXFOR C6011
	if (nodeG == NULL || nodeH == NULL || nodeF == NULL || nodeP == NULL)
	{
		Warning("Avoiding C6011 ai_pathfinder.cpp line 308\n");
		return NULL;
	}
	...
	openBS.Set(startID);
	//Msg("closeBS.Set(%i) at start of FindBestPath\n", startID);
	closeBS.Set( startID );

	// --------------- FIND BEST PATH ------------------
	while (!openBS.IsAllClear()) 
	{
		//Msg("Got to node loop in CAI_Pathfinder::FindBestPath\n");
		...
		{
			//Msg("End of node loop - Node %i skipped because incompatible hint group\n", smallestID);//ooh!! useful for later!
			continue;
		}

		if (smallestID == endID) 
		{
			//Msg("End of node loop - CAI_Pathfinder::FindBestPath success\n");
		...
		for (int link=0; link < pSmallestNode->NumLinks();link++) 
		{
			...
			//Msg("Got to link loop in CAI_Pathfinder::FindBestPath (%i, %i, node has %i links, on link number %i)\n", pSmallestNode->m_iID, pAInode[testID]->m_iID, pSmallestNode->NumLinks(), link);
			//If told not to use portals, but this link is through a portal, ditch.
			if (!IsLinkUsable(nodeLink, smallestID) || (bNoPortals && nodeLink->m_LinkInfo & bits_LINK_PORTAL))
			{
				//Msg("End of link loop - Skipped link because CAI_Pathfinder::IsLinkUsable returned FALSE\n");
				continue;
			}
			...
			if (dist == FLT_MAX)
			{
				//Msg("End of link loop - Skipped link because dist == FLT_MAX!?\n");
				continue;
			}

			float new_g  = nodeG[smallestID] + dist;

			//Msg("!closeBS.IsBitSet(testID) (%i) is %s, (new_g < nodeG[testID]) is %s (%0.1f, %f)\n", testID, !closeBS.IsBitSet(testID) ? "TRUE" : "FALSE", (new_g < nodeG[testID]) ? "TRUE" : "FALSE", new_g, nodeG[testID] == FLT_MAX ? 6969 : nodeG[testID]);
			// || nodeG[testID] == 0
			// || (new_g < nodeG[testID]) || nodeG[testID] == 0 //this tests if the new composite path dist is less than the current dist. this can cause problems with portals as they could return values that aren't really accurate for completely justified technical reasons. removing this from the if could cause poor pathfinding
			if (!closeBS.IsBitSet(testID))//PIN: Portal links can cause nodeG[testID] == 0 for uncertain reasons
			{
				//Msg("Link is possibly good.\nOLD P: %i G: %0.1f H: %0.1f F: %0.1f\n", nodeP[testID], nodeG[testID] == FLT_MAX ? 6969 : nodeG[testID], nodeH[testID], nodeF[testID]);
				...
				//Msg("NEW P: %i G: %0.1f H: %0.1f F: %0.1f\n", nodeP[testID], nodeG[testID] == FLT_MAX ? 6969 : nodeG[testID], nodeH[testID], nodeF[testID]);
				//Msg("closeBS.Set(%i) at end of FindBestPath\n", testID);
				closeBS.Set( testID );
				openBS.Set( testID );
			}
			//Msg("End of link loop\n");
		}
		//Msg("End of node loop\n");
	}
	//Msg("CAI_Pathfinder::FindBestPath failed\n");
	return NULL;   
}
...
AI_Waypoint_t* CAI_Pathfinder::FindShortRandomPath(int startID, float minPathLength, const Vector &directionIn) 
{
	...
	MARK_TASK_EXPENSIVE();
	//FIXFOR C6255
	int *nodeParent	= (int *)stackmalloc( sizeof(int) * nNodes );
	CVarBitVec closeBS(nNodes);
	...
	// ------------------------------------------
	//FIXFOR C6011
	if (pAInode[startID]->NumLinks() == 0 || nodeParent == NULL)
	{
		return NULL;
	}
			...
			(neighborID != NO_NODE && pAInode[neighborID]->GetType() == NODE_CLIMB))
	{
		...
		if (vDirection != vec3_origin)
		{
			...
			if (lastID == NO_NODE)
			{
				vLastPos = GetAbsOrigin();//PINCHANGED
			}
			else
			{
...
{
	//Msg("CAI_Pathfinder::IsLinkUsable testing link from %i to %i\n", startID, pLink->DestNodeID(startID));
	// --------------------------------------------------------------------------
	// Skip if link turned off
	// --------------------------------------------------------------------------
	if (pLink->m_LinkInfo & bits_LINK_OFF)
	{
		//Msg("CAI_Pathfinder::IsLinkUsable says link is turned off (not immediate failure)\n");
		CAI_DynamicLink *pDynamicLink = pLink->m_pDynamicLink;

		if (!pDynamicLink || pDynamicLink->m_strAllowUse == NULL_STRING)
		{
			//Msg("CAI_Pathfinder::IsLinkUsable says we can't be used because no CAI_DynamicLink\n");
			return false;
		}
	...
	//Msg("CAI_Pathfinder::IsLinkUsable linkMoveTypes is %i\n", linkMoveTypes);
	//Msg("CAI_Pathfinder::IsLinkUsable CapabilitiesGet() is %i\n", CapabilitiesGet());
	//Msg("CAI_Pathfinder::IsLinkUsable moveType is %i\n", moveType);
	...
	if (!moveType)
	{
		//Msg("CAI_Pathfinder::IsLinkUsable found no movetype\n");
		return false;
	}
	...
	// --------------------------------------------------------------------------
	if (GetOuter()->IsUnusableNode(endID, pEndNode->GetHint()))//PIN: redundant check?
	{
		//Msg("CAI_Pathfinder::IsLinkUsable failed because CAI_BaseNPC::IsUnusableNode found incompatible hint groups\n");
		return false;
	}	
	...
	{	
		if (!GetOuter()->IsJumpLegal(pStartNode->GetPosition(GetHullType()), 
									 ...
									 pEndNode->GetPosition(GetHullType())))
		{
			//Msg("CAI_Pathfinder::IsLinkUsable failed because CAI_BaseNPC::IsJumpLegal said jump is impossible\n");
			return false;
		}
	}
	...
	if (pLink->m_LinkInfo & bits_LINK_STALE_SUGGESTED)
	{
		...
		{
			//Msg("CAI_Pathfinder::IsLinkUsable failed because stale link\n");
			return false;
		}
	}
	//Msg("CAI_Pathfinder::IsLinkUsable succeeded\n");
	return true;
}
...
{
	//Msg("CAI_Pathfinder::RouteToNode marker 1\n");
	...
	{
		//Msg("CAI_Pathfinder::RouteToNode marker 2\n");
	...
	if (pResult)
	{
		//Msg("CAI_Pathfinder::RouteToNode marker 3\n");
		pResult->iNodeID = nodeID;
	}

	if (pResult != NULL && pResult->GetNext() == NULL)
	{
		//Warning("FRAUD WAYPOINT WITH NO NEXT 5\n");
	}

	//Msg("CAI_Pathfinder::RouteToNode marker 4\n");
	return pResult;
}
...
{
	//Msg("CAI_Pathfinder::RouteFromNode marker 1\n");
	...
	if (vecOrigin == vecNodePosition)
	{
		//Msg("CAI_Pathfinder::RouteFromNode marker 2\n");
		return CreateNodeWaypoint( GetHullType(), nodeID, bits_WP_TO_GOAL );
	}
	...
	{
		//Msg("CAI_Pathfinder::RouteFromNode marker 3\n");
	...
	int buildFlags, float flYaw, float goalTolerance, float maxLocalNavDistance )
{
	...
	{
		//Msg("CAI_Pathfinder::BuildComplexRoute marker 1\n");
		return new AI_Waypoint_t( vEnd, flYaw, navType, endFlags, nodeID );
	}
	...
	{
		//Msg("CAI_Pathfinder::BuildComplexRoute marker 1a\n");
		AIMoveTrace_t moveTrace;
		...
		{
			// It worked so return a route of length one to the endpoint
			//Msg("CAI_Pathfinder::BuildComplexRoute marker 2\n");
			return new AI_Waypoint_t( vEnd, flYaw, navType, endFlags, nodeID );
		}
		else
		{
			//Warning("CAI_Pathfinder::BuildComplexRoute marker 2a - BLOCKED\n");
		}
		...
		{
			//Msg("CAI_Pathfinder::BuildComplexRoute marker 3\n");
			return new AI_Waypoint_t( vEnd, flYaw, navType, endFlags, nodeID );
		}
		...
		{
			//Msg("CAI_Pathfinder::BuildComplexRoute marker 3a\n");
			if ( !UseStrongOptimizations() || ( GetOuter()->GetState() == NPC_STATE_SCRIPT || GetOuter()->IsCurSchedule( SCHED_SCENE_GENERIC, false ) ) )
			{
				//Msg("CAI_Pathfinder::BuildComplexRoute marker 3b\n");
				//FIXFOR C6246 - Why was this even being done in the first place? Isn't this just the same calculation twice?
				//float flTotalDist = ComputePathDistance( navType, vStart, vEnd );
				...
				if (triangRoute)
				{
					//Msg("CAI_Pathfinder::BuildComplexRoute marker 4\n");
					return triangRoute;
				}
			}
		}
		...
		{
			//Msg("CAI_Pathfinder::BuildComplexRoute marker 4a\n");
			...
			{
				//Msg("CAI_Pathfinder::BuildComplexRoute marker 4b\n");
				...
				{
					//Msg("CAI_Pathfinder::BuildComplexRoute marker 5\n");
					return new AI_Waypoint_t( vEnd, flYaw, navType, endFlags, nodeID );
				}
			}
		}
	}
	//Warning("CAI_Pathfinder::BuildComplexRoute marker 6 - FAILED\n");
	return NULL;
}
	  ...
	  Navigation_t navType)
{
	...
	{
		//Msg("CAI_Pathfinder::BuildTriangulationRoute marker 1\n");
		return NULL;
	}
	...
	//Msg("CAI_Pathfinder::BuildTriangulationRoute marker 2\n");
	return waypoint1;
}
...
{
	//Msg("CAI_Pathfinder::BuildLocalRoute running with vStart %0.1f %0.1f %0.1f, vEnd %0.1f %0.1f %0.1f\n",
	//	vStart[0], vStart[1], vStart[2], vEnd[0], vEnd[1], vEnd[2]);
	...
	if (buildFlags & bits_BUILD_GROUND)
	{
		...
		if (groundRoute)
		{
			...
			if (groundRoute != NULL && groundRoute->GetNext() == NULL)
			{
				//Warning("FRAUD WAYPOINT WITH NO NEXT 3\n");
			}
...
AI_Waypoint_t *CAI_Pathfinder::BuildRoute(const Vector &vStart, const Vector &vEnd, CBaseEntity *pTarget, float goalTolerance, bool bLocalSucceedOnWithinTolerance, Navigation_t curNavType, bool bNoPortals)
{
	//Msg("CAI_Pathfinder::BuildRoute %0.1f %0.1f %0.1f to %0.1f %0.1f %0.1f\n", vStart.x, vStart.y, vStart.z, vEnd.x, vEnd.y, vEnd.z);
	Assert(vStart.Length());
	Assert(vEnd.Length());
	int buildFlags = 0;
	...
	{
		//Msg("CAI_Pathfinder::BuildRoute marker 2\n");
	...
	{
		//Msg("CAI_Pathfinder::BuildRoute marker 6\n");
		buildFlags |= bits_BUILD_GET_CLOSE;
	}

	AI_Waypoint_t *pResult = NULL;

	//HACK: Chell thinks she can use local nav to phase through the chamber 01 glass. We should not have to do this.
	trace_t tr;
	UTIL_TraceLine(vStart + Vector(0, 0, 18), vEnd + Vector(0, 0, 18), MASK_NPCSOLID, UTIL_GetLocalPlayer(), COLLISION_GROUP_DEBRIS, &tr);
	if (tr.fraction != 1)
	{
		//Warning("Local nav impossible - quick reject\n");
		bTryLocal = false;
	}
	...
	{
		//Msg("CAI_Pathfinder::BuildRoute marker 7\n");
	...
	if ( !pResult )
	{
		//Msg("CAI_Pathfinder::BuildRoute marker 8 (trying node route instead of local)\n");
		pResult = BuildNodeRoute( vStart, vEnd, buildFlags, goalTolerance, bNoPortals);
	}
	...
	if (pResult != NULL && pResult->GetNext() == NULL)
	{
		//Warning("FRAUD WAYPOINT WITH NO NEXT\n");
	}
...
public:
	...
	bool IsValid( CAI_Node *pNode )
	{
		...
		{
			//Msg("CPathfindNearestNodeFilter::IsValid marker 6\n");
			return false;
		}
		...
		if (m_bToNode)
		{
			//Msg("CPathfindNearestNodeFilter::IsValid marker 7\n");
			m_pRoute = m_pPathfinder->RouteToNode(m_vGoal, buildFlags, pNode->GetId(), m_goalTolerance);
		}
		else
		{
			//Msg("CPathfindNearestNodeFilter::IsValid marker 8\n");
			m_pRoute = m_pPathfinder->RouteFromNode(m_vGoal, buildFlags, pNode->GetId(), m_goalTolerance);
		}

		m_nTries++;

		//Msg("CPathfindNearestNodeFilter::IsValid marker 9 (%s)\n", m_pRoute != NULL ? "TRUE" : "FALSE");
...
AI_Waypoint_t *CAI_Pathfinder::BuildNearestNodeRoute( const Vector &vGoal, bool bToNode, int buildFlags, float goalTolerance, int *pNearestNode )
{
	...
	//Msg("CAI_Pathfinder::BuildNearestNodeRoute running with vGoal %0.1f %0.1f %0.1f\n", vGoal[0], vGoal[1], vGoal[2]);
	CPathfindNearestNodeFilter filter( this, vGoal, bToNode, buildFlags, goalTolerance );
	*pNearestNode = GetNetwork()->NearestNodeToPoint(GetOuter(), vGoal, true, &filter, false);

	if (filter.m_pRoute != NULL && filter.m_pRoute->GetNext() == NULL)
	{
		//Warning("FRAUD WAYPOINT WITH NO NEXT 4\n");
	}
...
AI_Waypoint_t *CAI_Pathfinder::BuildNodeRoute(const Vector &vStart, const Vector &vEnd, int buildFlags, float goalTolerance, bool bNoPortals)
{
	AI_PROFILE_SCOPE( CAI_Pathfinder_BuildNodeRoute );
	//Msg("CAI_Pathfinder::BuildNodeRoute running with vStart %0.1f %0.1f %0.1f, vEnd %0.1f %0.1f %0.1f\n", 
	//	vStart[0], vStart[1], vStart[2], vEnd[0], vEnd[1], vEnd[2]);
	// ----------------------------------------------------------------------
	...
	//Msg("nearest source node: %d\n", srcID);
	if ( !srcRoute )
	{
		//Msg("BUILD NEAREST SOURCE NODE ROUTE FAILED. Node ID returned is %d\n", srcID );
		return NULL;
	}
	...
	{
		DeleteAll( srcRoute );
		//Msg("CAI_Pathfinder::BuildNodeRoute could not BuildNearestNodeRoute to dest %i\n", destID);
		return NULL;
	}
	...
	if (destID == srcID)
	{
		...
		//Msg("Node pathfind succeeded: dest == source (%i)\n", srcID);
		return srcRoute;
	}

	// If nodes are not connected by network graph, no route is possible
	if (!GetNetwork()->IsConnected(srcID, destID, GetOuter()))
	{
		//Msg("Network does not think nodes %i and %i are connected\n", srcID, destID);
		return NULL;
	}

	AI_Waypoint_t *path = FindBestPath(srcID, destID, bNoPortals);

	if (!path)
	{
		DeleteAll(srcRoute);
		DeleteAll(destRoute);
		//Msg("CAI_Pathfinder::BuildNodeRoute failed because CAI_Pathfinder::FindBestPath found no route between %i and %i\n", srcID, destID );
		return NULL;
	}
	else
	{
		//Msg("CAI_Pathfinder::BuildNodeRoute success with route between %i and %i\n", srcID, destID);
	}
	...
	if (srcRoute != NULL && srcRoute->GetNext() == NULL)
	{
		//Warning("FRAUD WAYPOINT WITH NO NEXT 2\n");
	}
	...
	float flDistToBlocker, const CBaseEntity *pTargetEnt, Vector *pApex )
{
	...
	{
		//Msg("CAI_Pathfinder::Triangulate marker 1\n");
		return false;
	}
	...
	{
		//Msg("CAI_Pathfinder::Triangulate marker 2\n");
		...
		{
			//Msg("CAI_Pathfinder::Triangulate marker 3\n");
			return false;
		}
	...
	{
		//Msg("CAI_Pathfinder::Triangulate marker 4\n");
		vecEnd = vecEndIn;
	}
	...
	{
		//Msg("CAI_Pathfinder::Triangulate marker 5\n");
		vecUp.Init( 0, 0, 1 );
		CrossProduct( vecForward, vecUp, vecPerpendicular );	// Orthogonal to facing
	}
	else
	{
		//Msg("CAI_Pathfinder::Triangulate marker 6\n");
		vecUp.Init( 0, 1, 0 );
	...
	if (GetOuter()->m_debugOverlays & OVERLAY_NPC_TRIANGULATE_BIT) 
	{
		//Msg("CAI_Pathfinder::Triangulate marker 7\n");
	...
	{
		//Msg("CAI_Pathfinder::Triangulate marker 8\n");
	...
	if (navType == NAV_FLY)
	{
		//Msg("CAI_Pathfinder::Triangulate marker 9\n");
	...
	for (int i = 0; i < 2; ++i )
	{
		...
		for (int j = nNumToTest; --j >= 0; )
		{
			...
			if (IsMoveBlocked(moveTrace))
			{
				//Msg("CAI_Pathfinder::Triangulate marker 13\n");
				...
				{
					//Msg("CAI_Pathfinder::Triangulate marker 14\n");
			...
			if (j & 0x1)
			{
				//Msg("CAI_Pathfinder::Triangulate marker 15\n");
				vecApex[j] += vecDelta[j >> 1];
			}
			else
			{
				//Msg("CAI_Pathfinder::Triangulate marker 16\n");
				vecApex[j] -= vecDelta[j >> 1];
			}
		}
	}

	//Msg("CAI_Pathfinder::Triangulate marker 17\n");
	return false;
}
